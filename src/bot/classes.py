...
class SessionGuild(typing.TypedDict):
	name: str
	id: str
	icon: str

class SessionVoice(typing.TypedDict):
	name: str
	id: str
...
class CustomPlayer(BasePlayer):
	...
	async def play(self, track: typing.Union[AudioTrack, dict] = None, start_time: int = 0, end_time: int = 0, no_replace: bool = False, channel = None, guild = None):
		"""
		...
		self.current = track
		await self.node._send(op='play', guildId=str(self.guild_id), track=track.track, **options)
	...
	async def set_gains(self, *gain_list):
		...
		await self.node._send(op='equalizer', guildId=str(self.guild_id), bands=update_package)
	...
	async def change_node(self, node):
		...
		if self.current:
			await self.node._send(op='play', guildId=str(self.guild_id), track=self.current.track, startTime=self.position)
			self._last_update = time() * 1000
		...
		if any(self.equalizer):  # If any bands of the equalizer was modified
			...
			await self.node._send(op='equalizer', guildId=str(self.guild_id), bands=payload)
